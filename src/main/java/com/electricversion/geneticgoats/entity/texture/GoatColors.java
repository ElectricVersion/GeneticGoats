package com.electricversion.geneticgoats.entity.texture;

public class GoatColors {
    private int whiteColor = -1;

    private int noseRedColor = -1;
    private int noseBlackColor = -1;
    private int noseWhiteColor = -1;

    public int getNoseRedColor() {
        return noseRedColor;
    }

    public void setNoseRedColor(int noseRedColor) {
        this.noseRedColor = noseRedColor;
    }

    public int getNoseBlackColor() {
        return noseBlackColor;
    }

    public void setNoseBlackColor(int noseBlackColor) {
        this.noseBlackColor = noseBlackColor;
    }

    public int getNoseWhiteColor() {
        return noseWhiteColor;
    }

    public void setNoseWhiteColor(int noseWhiteColor) {
        this.noseWhiteColor = noseWhiteColor;
    }

    public int getWhiteColor() {
        return whiteColor;
    }

    public void setWhiteColor(int whiteColor) {
        this.whiteColor = whiteColor;
    }
}
